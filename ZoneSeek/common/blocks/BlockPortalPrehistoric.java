...
public class BlockPortalPrehistoric extends BlockPortal
{
	private String textureName;

	public BlockPortalPrehistoric(int par1, String tex)
	{
			super(par1);
	...
	public boolean tryToCreatePortal(World par1World, int par2, int par3, int par4)
    {
        ...
        if (par1World.getBlockId(par2 - 1, par3, par4) == BlocksHelper.SlagBlock.blockID|| par1World.getBlockId(par2 + 1, par3, par4) == BlocksHelper.SlagBlock.blockID)
        {
            b0 = 1;
        }

        if (par1World.getBlockId(par2, par3, par4 - 1) == BlocksHelper.SlagBlock.blockID || par1World.getBlockId(par2, par3, par4 + 1) == BlocksHelper.SlagBlock.blockID)
        {
            b1 = 1;
        }
        ...
        else
        {
            ...
            for (l = -1; l <= 2; ++l)
            {
                for (i1 = -1; i1 <= 3; ++i1)
                {
                    ...
                    if (l != -1 && l != 2 || i1 != -1 && i1 != 3)
                    {
                        ...
                        if (flag)
                        {
                            if (j1 != BlocksHelper.SlagBlock.blockID)
                            {
                                return false;
                            }
                        }
            ...
            for (l = 0; l < 2; ++l)
            {
                for (i1 = 0; i1 < 3; ++i1)
                {
                    par1World.setBlock(par2 + b0 * l, par3 + i1, par4 + b1 * l, BlocksHelper.portalprehistoric.blockID, 0, 2);
                }
            }
    ...
    public void onNeighborBlockChange(World par1World, int par2, int par3, int par4, int par5)
    {
        ...
        if (par1World.getBlockId(par2, i1 - 1, par4) != BlocksHelper.SlagBlock.blockID)
        {
        ...
        else
        {
            ...
            if (j1 == 3 && par1World.getBlockId(par2, i1 + j1, par4) == BlocksHelper.SlagBlock.blockID)
            {
                ...
                {
                    if ((par1World.getBlockId(par2 + b0, par3, par4 + b1) != BlocksHelper.SlagBlock.blockID || par1World.getBlockId(par2 - b0, par3, par4 - b1) != this.blockID) && (par1World.getBlockId(par2 - b0, par3, par4 - b1) != BlocksHelper.SlagBlock.blockID || par1World.getBlockId(par2 + b0, par3, par4 + b1) != this.blockID))
                    {
                        par1World.setBlockToAir(par2, par3, par4);
                    }
                }
    ...
    public void onEntityCollidedWithBlock(World par1World, int par2, int par3, int par4, Entity par5Entity)
    {
        if (par5Entity.ridingEntity == null && par5Entity.riddenByEntity == null)
        {
        	...
        	if(par5Entity.timeUntilPortal == 0 && par5Entity instanceof EntityPlayerMP)
        	{
                ...
                WorldServer worldserver1 = minecraftserver.worldServerForDimension(IDsHelper.PrehistoricAgeID);
                if(dimID == IDsHelper.PrehistoricAgeID)
                {
                	((EntityPlayerMP)par5Entity).sendChatToPlayer("Moving from  " + dimID + " to OverWorld");
                	minecraftserver.getConfigurationManager().transferPlayerToDimension((EntityPlayerMP) par5Entity, 0, new PrehistoricTeleporter(worldserver));
                    
                } else {
                	((EntityPlayerMP)par5Entity).sendChatToPlayer("Moving from " + dimID + " to the Prehistoric Age");
                	minecraftserver.getConfigurationManager().transferPlayerToDimension((EntityPlayerMP) par5Entity, IDsHelper.PrehistoricAgeID, new PrehistoricTeleporter(worldserver1));
